...
from ..evaluator.constrastive import ConstrastiveEvaluator
from lib.utils.convert import unnormalize


class SuperPoint(nn.Module):
    weight_path = 'data/model/superpoint_v1.pth'
    def __init__(self):
        super(SuperPoint, self).__init__()
        if not os.path.exists(self.weight_path):
            print('You should first download superpoint weight to "data/model/superpoint_v1.pth".')
            print('Superpoint repo: https://github.com/MagicLeapResearch/SuperPointPretrainedNetwork')
            sys.exit()
        
        self.feature_extractor = SuperPointFeature()
        self.feature_extractor.load_state_dict(torch.load(self.weight_path))
        self.evaluator = ConstrastiveEvaluator()
